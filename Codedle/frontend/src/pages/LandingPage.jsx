...
import { Terminal, UserPlus, Calendar, Timer, Trophy } from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="bg-background text-on-surface">
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-8 md:px-24 py-4 border-b border-surface-outline bg-surface/60 backdrop-blur-xl">
        ...
        {isAuthenticated ? (
          <nav className="hidden md:flex items-center gap-8">
            <Link className="font-label-md text-label-md text-primary hover:text-primary-dim transition-colors" to="/terminal">
              Terminal
            </Link>
            <Link className="font-label-md text-label-md text-secondary hover:text-primary transition-colors" to="/leaderboard">
              Leaderboard
            </Link>
            <Link className="font-label-md text-label-md text-secondary hover:text-primary transition-colors" to="/time-attack">
              Time Attack
            </Link>
            <Link className="font-label-md text-label-md text-secondary hover:text-primary transition-colors" to="/settings">
              Settings
            </Link>
          </nav>
        ) : (
          <nav className="hidden md:flex items-center gap-8">
            <Link className="font-label-md text-label-md text-primary hover:text-primary-dim transition-colors" to="/auth?tab=login">
              Login
            </Link>
            <Link className="font-label-md text-label-md text-secondary hover:text-primary transition-colors" to="/auth?tab=signup">
              Signup
            </Link>
          </nav>
        )}
        <div className="flex items-center gap-4">
          {isAuthenticated ? (
            <>
              ...
              <Link className="bg-primary text-primary-foreground px-6 py-2 rounded-lg font-label-md text-label-md font-bold hover:brightness-110 transition-all shadow-sm" to="/terminal">
                DAILY_CHALLENGE
              </Link>
            </>
          ) : (
            <>
              <Link className="bg-primary text-primary-foreground px-6 py-2 rounded-lg font-label-md text-label-md font-bold hover:brightness-110 transition-all shadow-sm" to="/auth?tab=login">
                Login
              </Link>
              <Link className="bg-surface-paper text-secondary border border-surface-outline px-6 py-2 rounded-lg font-label-md text-label-md font-bold hover:bg-surface-muted transition-colors" to="/auth?tab=signup">
                Signup
              </Link>
            </>
          )}
      ...
      <main className="pt-24">
        <section className="relative min-h-[819px] flex flex-col items-center justify-center text-center px-margin py-20 overflow-hidden">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary/10 rounded-full blur-3xl -z-20"></div>
          <div className="max-w-4xl space-y-6">
            <FadeIn delay={0.1}>
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary-container/40 border border-primary/20 rounded-full mb-4">
                <span className="w-2 h-2 rounded-full bg-primary node-pulse"></span>
                <span className="font-label-sm text-label-sm text-primary-dim tracking-widest uppercase">
                  System Online: Challenge #412
                </span>
              </div>
            </FadeIn>
            <FadeIn delay={0.2}>
              <h1 className="font-headline-xl text-headline-xl md:text-[80px] text-on-surface leading-tight">
            ...
            <FadeIn delay={0.4}>
              <div className="flex flex-col md:flex-row items-center justify-center gap-4 mt-10">
                <Link className="bg-primary text-primary-foreground px-10 py-4 rounded-xl font-headline-md text-headline-md shadow-lg hover:scale-[1.02] hover:brightness-110 transition-all flex items-center gap-3" to="/terminal">
                  Start Daily Challenge
                  <Terminal size={20} />
                </Link>
                <Link className="bg-surface-paper text-secondary border border-surface-outline px-10 py-4 rounded-xl font-headline-md text-headline-md hover:bg-surface-muted transition-colors" to="/auth?tab=login">
                  Login
                </Link>
                <Link className="bg-primary text-primary-foreground px-10 py-4 rounded-xl font-headline-md text-headline-md shadow-lg hover:scale-[1.02] hover:brightness-110 transition-all flex items-center gap-3" to="/auth?tab=signup">
                  Signup
                  <UserPlus size={20} />
                </Link>
              </div>
        ...
        <section className="px-margin py-20 max-w-container-max mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
            <FadeIn delay={0.5} direction="right">
              <div className="md:col-span-7 bg-surface-paper border border-surface-outline rounded-xl p-8 relative overflow-hidden h-[500px] glass-card">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="font-headline-md text-headline-md text-primary">System Pipeline</h3>
                  <span className="font-label-md text-label-md text-secondary bg-surface-muted px-3 py-1 rounded">
                ...
                <div className="relative h-full flex flex-col justify-center items-center">
                  <svg className="absolute w-full h-full pointer-events-none" viewBox="0 0 600 300">
                    ...
                    <path className="pipeline-flow" d="M50,150 L150,150 L250,80 L350,220 L450,150 L550,150" fill="none" stroke="#10b981" strokeDasharray="10,500" strokeWidth="4"></path>
                  </svg>
                  <div className="absolute left-10 top-1/2 -translate-y-1/2 flex flex-col items-center">
                    <div className="w-12 h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold">01</div>
                  ...
                  <div className="absolute right-10 top-1/2 -translate-y-1/2 flex flex-col items-center">
                    <div className="w-12 h-12 rounded-full bg-surface-variant flex items-center justify-center font-bold opacity-50">04</div>
        ...
        <section className="bg-surface-muted py-24 px-margin">
          <div className="max-w-container-max mx-auto">
            <FadeIn delay={0.7}>
              <div className="text-center mb-16 space-y-4">
                <h2 className="font-headline-lg text-headline-lg text-on-surface">Engineered for Accuracy</h2>
                <p className="font-body-md text-body-md text-secondary">The definitive mental gym for software engineers.</p>
              </div>
            </FadeIn>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <FadeIn key={item.title} delay={0.8 + idx * 0.1}>
                  <div className="bg-surface-paper p-8 rounded-xl border border-surface-outline hover:border-primary transition-all group glass-card">
                    ...
                    <p className="font-body-md text-body-md text-secondary">{item.text}</p>
                  </div>
                </FadeIn>
              ))}
            </div>
        ...
        <section className="py-12 px-margin border-y border-surface-outline bg-surface-paper overflow-hidden">
      ...
      <footer className="bg-surface-paper py-16 px-margin">
        <div className="max-w-container-max mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="space-y-4">
            ...
            <p className="font-label-sm text-label-sm text-secondary max-w-xs">
        ...
        <div className="max-w-container-max mx-auto mt-16 pt-8 border-t border-surface-outline flex justify-between items-center">
